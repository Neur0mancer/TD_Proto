        ...
        if( Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject())
        {
            if (activeBuildingType != null){
                if (CanSpawnBuilding(activeBuildingType, UtilsClass.GetMouseWorldPosition(), out string errorMessage))
                {
                    ...
                    else { TooltipUI.Instance.Show("Can't afford" + "\n" + activeBuildingType.GetConstructionResourceCostString(),
                        new TooltipUI.TooltipTimer { timer = 2f }); }
                }
                else { TooltipUI.Instance.Show(errorMessage, new TooltipUI.TooltipTimer { timer = 2f}); }
            }
    ...
    private bool CanSpawnBuilding(BuildingTypeSO buildingType, Vector3 position, out string errorMessage)
    {
        ...
        foreach(Collider2D collider in collider2DArray)
        {
            ...
            if(buildingTypeHolder != null)
            {
                ...
                {
                    errorMessage = "Too close to other building!";
                    return false;
                }
            }
        }
